...
class MTMLKD(MultiTeacherDistiller):
    def __init__(self, student, teachers, cfg):
        super(MTMLKD, self).__init__(student, teachers)
    ...
    def forward_train(self, image_weak, image_strong, target, **kwargs):
        ...
        logits_teachers_weak  = torch.zeros_like(logits_student_weak)
        logits_teachers_strong  = torch.zeros_like(logits_student_strong)
        for teacher in self.teachers:
            with torch.no_grad():
                logits_teacher_weak, _ = teacher(image_weak)
                logits_teacher_strong, _ = teacher(image_strong)
                logits_teachers_weak = logits_teachers_weak + logits_teacher_weak
                logits_teachers_strong = logits_teachers_strong + logits_teacher_strong
        

        logits_teacher_weak  = logits_teachers_weak/self.num_teachers
        logits_teacher_strong  = logits_teachers_strong/self.num_teachers
