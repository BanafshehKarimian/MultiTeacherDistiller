...
import copy
import random
class DoubleMLKD(Distiller):
    def __init__(self, student, teacher, cfg):
        super(DoubleMLKD, self).__init__(student, teacher)
    ...
    def forward_train(self, image_weak, image_strong, target, **kwargs):
        if random.random() < 0.5:
            ...
            logits_student_weakB, _ = self.studentB(image_weak)
            logits_student_strongB, _ = self.studentB(image_strong)
        else:
            with torch.no_grad():
                logits_student_weakB, _ = self.studentB(image_weak)
                logits_student_strongB, _ = self.studentB(image_strong)
        ...
        logits_student_weak = (logits_student_weak + logits_student_weakB)/2
        logits_student_strong = (logits_student_strong + logits_student_strongB)/2
