...
def main(cfg, resume, opts):
    ...
    elif cfg.DISTILLER.TYPE == 'DoubleMLKD':
        train_loader, val_loader, num_data, num_classes = get_dataset_strong(cfg)
    elif cfg.DISTILLER.TYPE == 'MTMLKD':
        train_loader, val_loader, num_data, num_classes = get_dataset_strong(cfg)
    else:
    ...
    # distillation
        ...
        if cfg.DISTILLER.MULTITEACHER:
            model_teacher = [deepcopy(model_teacher).cuda()]
            for teacher in cfg.DISTILLER.OTHERTEACHERS:
                net, pretrain_model_path = cifar_model_dict[teacher]
                assert (
                    pretrain_model_path is not None
                ), "no pretrain model for teacher {}".format(cfg.DISTILLER.TEACHER)
                model_other_teacher = net(num_classes=num_classes)
                model_other_teacher.load_state_dict(load_checkpoint(pretrain_model_path)["model"])
                model_teacher.append(deepcopy(model_other_teacher).cuda())
        if cfg.SOLVER.TRAINER == "MTS":
            distiller = []
            for teacher in model_teacher:
                distiller_ = distiller_dict[cfg.DISTILLER.TYPE](
                    deepcopy(model_student), teacher, cfg
                )
                distiller_ = torch.nn.DataParallel(distiller_.cuda())
                distiller.append(distiller_)
...
if __name__ == "__main__":
    ...
    if args.logit_stand:# and cfg.DISTILLER.TYPE in ['KD','DKD','MLKD']:
        ...
        if cfg.DISTILLER.TYPE == 'DoubleKD':
            cfg.KD.LOSS.KD_WEIGHT = args.kd_weight
            cfg.KD.TEMPERATURE = args.base_temp
        if cfg.DISTILLER.TYPE == 'MTKD':
            cfg.KD.LOSS.KD_WEIGHT = args.kd_weight
            cfg.KD.TEMPERATURE = args.base_temp
        ...
        elif cfg.DISTILLER.TYPE == 'DoubleDKD':
            cfg.DKD.ALPHA = cfg.DKD.ALPHA * args.kd_weight
            cfg.DKD.BETA = cfg.DKD.BETA * args.kd_weight
            cfg.DKD.T = args.base_temp
        elif cfg.DISTILLER.TYPE == 'MTDKD':
            cfg.DKD.ALPHA = cfg.DKD.ALPHA * args.kd_weight
            cfg.DKD.BETA = cfg.DKD.BETA * args.kd_weight
            cfg.DKD.T = args.base_temp
